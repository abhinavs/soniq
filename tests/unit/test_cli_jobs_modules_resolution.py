"""
Test that CLI job-module resolution reads SONIQ_JOBS_MODULES fresh, honours a
value set only in .env, and stays independent of the global settings cache.
"""
...
def test_dotenv_is_read_from_the_working_directory(monkeypatch, tmp_path):
    """The .env beside the operator's project is the one that counts.

    Guards the ``dotenv_values(".env")`` path argument: a bare
    ``dotenv_values()`` searches upward from this module's location inside the
    installed package, which would silently ignore the project's own file.
    """
    monkeypatch.delenv("SONIQ_JOBS_MODULES", raising=False)
    project = tmp_path / "project"
    project.mkdir()
    (project / ".env").write_text("SONIQ_JOBS_MODULES=project.tasks\n")
    monkeypatch.chdir(project)

    assert resolve_jobs_modules(_args()) == ["project.tasks"]
...
def test_missing_dotenv_file_is_not_an_error(monkeypatch, tmp_path):
    """Most deployments export the var and ship no .env at all."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("SONIQ_JOBS_MODULES", "app.tasks")
